...
function accordion() {
  ...
  for (i = 0; i < acc.length; i++) {
      acc[i].addEventListener("click", function() {
          ...
          var panel = this.nextElementSibling; // The associated content div
          var indicator = this.querySelector('.accordion-indicator'); // The + or − span

          // Toggle the active state for smooth accordion behavior
...
window.onload = function() {
  accordion(); // Directly initialize the accordion
};

document.addEventListener("DOMContentLoaded", function() {
  accordion(); // Initialize the accordion
});

function pageLoad() {
  toolTips();  // Assuming toolTips is another function in your code
  accordion(); // Initialize accordion functionality
}

window.onload = pageLoad; // Ensure that pageLoad runs when the page loads

//js function for the cross effect of the hamburger menu
const hamburger = document.getElementById('hamburger');
const menu = document.getElementById('menu');

hamburger.addEventListener('click', () => {
  // Toggle the 'active' class for the hamburger
